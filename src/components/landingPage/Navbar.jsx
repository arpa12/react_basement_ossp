...
import { Box, Flex, Text, Button, Switch, HStack, VStack, IconButton, Drawer, DrawerOverlay, DrawerContent, DrawerCloseButton, useDisclosure, Image } from '@chakra-ui/react';
...
const Navbar = () => {
    ...
    const renderContactInfo = () => (
        <HStack spacing={4}>
            {CONTACT_INFO.map((info, index) => (
                <HStack key={index}>
                    <info.icon />
                    <Text fontWeight="bold" fontSize="sm">{info.label}</Text>
                </HStack>
            ))}
        </HStack>
    );
    ...
    const renderLanguageSwitcher = () => (
        <HStack spacing={4}>
            <Text fontWeight="bold" fontSize="sm">{isEnglish ? 'EN' : 'বাং'}</Text>
    ...
    return (
        <>
            {/* Top Bar */}
            <Box bg="green.700" color="white" px={{ base: 4, md: 8 }} py={5}>
            ...
            {/* Main Navbar */}
            <Box bg="white" color="black" px={{ base: 4, md: 8 }} py="2px" shadow="md">
                <Flex alignItems="center" justifyContent="space-between" mx="auto" maxW="1200px">
                    <HStack spacing={2}>
                        <RouterLink to="/">
                            <Image
                    ...
                    <HStack spacing={6} display={{ base: 'none', md: 'flex' }}>
                        <Button as={RouterLink} to="/about" colorScheme="blue" variant="outline" leftIcon={<InfoOutlineIcon />}>
                            {isEnglish ? 'About OSSP' : 'ওএসএসপি সম্পর্কে'}
                        </Button>
                        <Button as={RouterLink} to="/why" colorScheme="blue" variant="outline" leftIcon={<QuestionOutlineIcon />}>
                            {isEnglish ? 'Why OSSP' : 'কেন ওএসএসপি'}
                        </Button>
                        <Button as={RouterLink} to="/login" colorScheme="blue" leftIcon={<LockIcon />}>
                            {isEnglish ? 'Login' : 'লগইন'}
                        </Button>
                        <Button as={RouterLink} to="/signup" colorScheme="blue" variant="solid">
